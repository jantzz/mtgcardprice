...
class CardPriceChecker():
    ...
    def __init__(self):
        ...
        # Frame for card searching and suggestions: 
        self.search_frame = tk.Frame(self.root)
        self.search_frame.pack(pady=20) # pack the frame to fill the window

        # label for searching cards: 
        self.search_label = tk.Label(self.search_frame, text="Search for a card:", font=("Arial", 16))
        self.search_label.grid(row=0, column=0) # pack the label to fill the window
        
        #entry for searching cards
        self.search_entry = tk.Entry(self.search_frame, font=("Arial", 16)) # create an entry widget for the card name
        self.search_entry.grid(row=0, column=1) # pack the entry widget to fill the window

        # button to start search 
        self.search_button = tk.Button(self.search_frame, text="Search", command=self.searchCard) # create a button to search for the card
        self.search_button.grid(row=0, column=2) # pack the button to fill the window

        # frame to display card information
        self.single_card = tk.Frame(self.root) # create a new frame for the card information
        self.single_card.pack() # pack the frame to fill the window
        self.single_card.pack_forget() # hide the frame at first

        #progress bar 
        self.progress_bar = ttk.Progressbar(self.root, orient="horizontal", mode="indeterminate") # create a progress bar widget
        self.progress_bar.pack(padx=10, pady=20) # pack the progress bar to fill the window
        self.progress_bar.pack_forget() #hide the progress bar at first 
    ...
    def display_single_Card(self, data: dict): #data expected to be a list object 
        ...
        for widget in self.single_card.winfo_children():
            widget.destroy()

        self.single_card.pack(fill="x")  # Make sure it's visible again

        self.card_name_label = tk.Label(self.single_card, text=data['Name'], font=("Arial", 16)) # create a label for the card name
        self.card_name_label.grid(row=0, column=0, sticky="nsew") # pack the label to fill the window

        self.card_set_label = tk.Label(self.single_card, text=data['Set'], font=("Arial", 16)) # create a label for the card set
        self.card_set_label.grid(row=1, column=0, sticky="nsew") # pack the label to fill the window

        self.card_price_label = tk.Label(self.single_card, text=data['Price (USD)'], font=("Arial", 16)) # create a label for the card price
        self.card_price_label.grid(row=2, column=0, sticky="nsew") # pack the label to fill the window

        self.card_foil_label = tk.Label(self.single_card, text=data['Price (Foil)'], font=("Arial", 16)) # create a label for the card foil price
        self.card_foil_label.grid(row=3, column=0, sticky="nsew") # pack the label to fill the window
        
        if(data['Date searched: '] != None):
            self.card_date_label = tk.Label(self.single_card, text=data['Date searched: '], font=("Arial", 16))
            self.card_date_label.grid(row=4, column=0, sticky="nsew") # pack the label to fill the window
        elif data['edhrec_rank'] != None:
            self.card_date_label = tk.Label(self.single_card, text=data['edhrec_rank'], font=("Arial", 16))
    ...
    def display_card_data(self): 
        ...
        if hasattr(self, 'card_list_frame'):
            self.card_list_frame.destroy() # destroy the previous card details if it exists

        self.card_list_frame = tk.Frame(self.root) # create a new frame for the card details
        self.card_list_frame.pack(expand=True) # pack the frame to fill the window
        ...
        self.display_card_info = ttk.Treeview(self.card_list_frame, columns=cols, show='headings') # create a treeview widget to display the card details
        #TODO sort by column, etc. 
        ...
        self.card_list_frame.columnconfigure(0, weight=1)
        self.card_list_frame.rowconfigure(0, weight=1)

        self.card_scrollbar = ttk.Scrollbar(self.card_list_frame, orient="vertical", command=self.display_card_info.yview) # create a vertical scrollbar for the treeview widget
        ...
        self.card_label = tk.Label(self.card_list_frame, text="Cards from file", font=("Arial", 16)) # create a label for the card details
        self.card_label.grid(row=0, column=0, sticky="nsew") # pack the label to fill the window

        self.card_separator = ttk.Separator(self.card_list_frame, orient="horizontal") # create a separator for the card details
    ...
    def display_popular_cards(self):
        ...
        if hasattr(self, 'popular_cards_frame'):
            self.popular_cards_frame.destroy()
        
        self.popular_cards_frame = tk.Frame(self.root) # create a new frame for the popular cards
        self.popular_cards_frame.pack(expand=True) # pack the frame to fill the window

        self.pop_cards = ttk.Treeview(self.popular_cards_frame, columns=('Name', 'Set', 'Price (USD)', 'Price (Foil)', 'EDHREC Rank'), show='headings') # create a treeview widget to display the popular cards
        ...
        self.popular_cards_frame.columnconfigure(0, weight=1)
        self.popular_cards_frame.rowconfigure(0, weight=1)

        self.popular_cards_scrollbar = ttk.Scrollbar(self.popular_cards_frame, orient="vertical", command=self.pop_cards.yview) # create a vertical scrollbar for the treeview widget
        ...
        self.pop_label = tk.Label(self.popular_cards_frame, text="Popular Cards", font=("Arial", 16)) # create a label for the popular cards
        self.pop_label.grid(row=0, column=0, sticky="nsew") # pack the label to fill the window

        self.pop_separator = ttk.Separator(self.popular_cards_frame, orient="horizontal") # create a separator for the popular cards
